...
def _fetch_sync(symbol: str, period: str, timeout_seconds: float) -> list[OHLCVRow]:
    ...
    # auto_adjust=True applies splits/dividends to OHLCV; we store both raw close
    # (close column) and adjusted close (auto_adjust gives the adjusted close in
    # the Close column — so we set auto_adjust=False and request actions separately
    # to preserve both).
    ...
    for ts, row in df.iterrows():
        ...
        # yfinance may return NaN for adj_close on non-adjusted runs — guard.
