...
Output structure:
    ...
    │   └── ...
    ├── news/
    │   ├── all.json             # Generic business feed
    │   ├── AAPL.json            # Per-symbol feed (equities + crypto)
    │   └── ...
    └── filings/
        ├── AAPL.json            # EDGAR filings (equities only)
        └── ...
...
# News sources used across mock feeds. Real publishers so the UI looks
# realistic even with synthetic content. No article URLs are real.
...
# Generic business-news headline templates. One placeholder per entry —
# {subject} is filled with either a ticker, a company name, or a generic
# market-wide subject for the "all.json" feed.
_HEADLINE_TEMPLATES: list[str] = [
    "{subject} shares rally on upbeat guidance",
    "{subject} beats quarterly estimates, raises outlook",
    "{subject} faces regulatory scrutiny over new product line",
    "{subject} announces strategic partnership with major cloud provider",
    "{subject} reports record free cash flow",
    "Analysts upgrade {subject} on margin expansion",
    "{subject} insiders sell into strength, filings show",
    "{subject} expands international operations",
    "{subject} CEO signals focus on AI-driven growth",
    "{subject} buyback program lifts sentiment",
]

# SEC filing form types and rotation weights. 8-K dominates filing volume
# because issuers must file them for material events (typical issuer
# files 4–8 8-Ks per year vs. 1 10-K + 3 10-Qs). Weights: 10-K=10%,
# 10-Q=30%, 8-K=60%.
_FILING_FORM_WEIGHTS: list[tuple[str, float]] = [
    ("10-K", 0.10),
    ("10-Q", 0.30),
    ("8-K", 0.60),
]
...
def _news_articles_for_subject(
    subject: str, symbol: str | None, n: int
) -> list[dict[str, object]]:
    """
    Generate *n* synthetic news articles for *subject* (ticker or phrase).

    Articles are dated within the last 14 days (newest first) to match the
    NewsAPI recency window. Each entry carries a non-real URL — mock_data/
    must not link to anything the browser could accidentally fetch.
    """
    articles: list[dict[str, object]] = []
    now = datetime.now(tz=timezone.utc)
    for i in range(n):
        template = _RNG.choice(_HEADLINE_TEMPLATES)
        source = _RNG.choice(_NEWS_SOURCES)
        # Space articles across the 14-day window, newest first.
        published = now - timedelta(
            days=_RNG.randint(0, 14),
            hours=_RNG.randint(0, 23),
            minutes=_RNG.randint(0, 59),
        )
        slug = template.split()[0].lower()
        articles.append(
            {
                "title": template.format(subject=subject),
                "description": (
                    f"Mock summary for {subject}. Generated by "
                    "scripts/generate_mock_data.py."
                ),
                "url": f"https://mock.invalid/news/{slug}/{i}",
                "published_at": published.isoformat(),
                "source_name": source,
                "symbol": symbol,
            }
        )
    # Newest first ordering matches the NewsAPI default sort.
    articles.sort(
        key=lambda a: str(a["published_at"]),
        reverse=True,
    )
    return articles


def _filings_for_symbol(symbol: str, n: int) -> list[dict[str, object]]:
    """
    Generate *n* synthetic EDGAR filings for *symbol*, newest first.

    Accession numbers follow the real EDGAR format
    (XXXXXXXXXX-YY-NNNNNN) but are entirely synthetic.
    """
    filings: list[dict[str, object]] = []
    today = date.today()
    for i in range(n):
        # Weighted random form type — matches real issuer filing cadence.
        roll = _RNG.random()
        cumulative = 0.0
        form_type = _FILING_FORM_WEIGHTS[0][0]
        for ft, weight in _FILING_FORM_WEIGHTS:
            cumulative += weight
            if roll <= cumulative:
                form_type = ft
                break

        # Filed date: within the last 24 months, newest first ordering
        # enforced after the loop.
        days_back = _RNG.randint(1, 730)
        filed_at = datetime(
            today.year, today.month, today.day, tzinfo=timezone.utc
        ) - timedelta(days=days_back)
        # Period of report: quarter or fiscal year end before filed date.
        period_of_report = (filed_at - timedelta(days=30)).date()
        accession = f"00003{_RNG.randint(10000, 99999)}-{filed_at.year % 100:02d}-{i:06d}"
        ...
        filings.append(
            {
                ...
                "filing_url": (
                    f"https://mock.invalid/filings/{symbol}/{accession}"
                ),
                "description": f"Mock {form_type} filing for {symbol}.",
            }
        )
    filings.sort(key=lambda f: str(f["filed_at"]), reverse=True)
    return filings
...
def generate() -> None:
    ...
    # ── News ──────────────────────────────────────────────────────────────
    # Generic "all" feed drives the unsymboled /news endpoint; per-symbol
    # feeds back /news/{symbol}. ADR-006 forbids network calls in mock mode.
    print("\n[news]")
    _write_json(
        _MOCK_DATA_DIR / "news" / "all.json",
        {
            "source": "mock",
            "articles": _news_articles_for_subject(
                subject="markets", symbol=None, n=30
            ),
        },
    )
    for entry in _EQUITIES + _CRYPTO:
        symbol = str(entry["symbol"])
        name = str(entry["name"])
        _write_json(
            _MOCK_DATA_DIR / "news" / f"{symbol}.json",
            {
                "source": "mock",
                "articles": _news_articles_for_subject(
                    subject=name, symbol=symbol, n=10
                ),
            },
        )

    # ── Filings ───────────────────────────────────────────────────────────
    # Filings are equities-only — SEC does not accept crypto issuer filings.
    print("\n[filings]")
    for equity in _EQUITIES:
        symbol = str(equity["symbol"])
        _write_json(
            _MOCK_DATA_DIR / "filings" / f"{symbol}.json",
            {
                "source": "mock",
                "filings": _filings_for_symbol(symbol, n=10),
            },
        )
    ...
    # Count: OHLCV (crypto + equity) + macro + news (all + per-symbol) +
    # filings (equity only) + instruments.json + quotes.json.
    news_file_count = 1 + len(_EQUITIES) + len(_CRYPTO)
    filing_file_count = len(_EQUITIES)
    total_files = (
        len(_CRYPTO)
        + len(_EQUITIES)
        + len(_MACRO_SERIES)
    ...
    print(f"  {news_file_count} news files (1 all + {len(_EQUITIES) + len(_CRYPTO)} per-symbol)")
    print(f"  {filing_file_count} filings files")
